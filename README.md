...
视频区顶部的“视觉”选项支持 `增强处理` 和 `不处理（对照）` 两组。对照组仍执行相同的工作分辨率缩放，以保持输入尺寸、坐标系和标定条件一致，但跳过框外白平衡、背景/保护环模糊和框内自适应增强。两组处理耗时应分别记录。画面打开时也可以切换；为避免同一次跟踪混入两种输入，切换后会暂停离线视频并清除旧目标，需要重新框选。电机正在运行或处于故障锁定时不能切换。
...
Windows 推荐使用仓库自带的一键启动器。电脑只需预先安装来自 [python.org](https://www.python.org/downloads/windows/) 的 64 位 Python 3.11～3.14（建议保留默认的 Python Launcher 和 Tcl/Tk 组件）：

```powershell
run_app.bat
```

也可以在资源管理器中直接双击 `run_app.bat`。第一次运行会自动查找兼容的 Python、创建 `.venv` 并安装依赖；后续直接启动 APP。只有 `requirements.txt` 发生变化时才会再次更新依赖，不需要手动激活虚拟环境。仓库路径可以包含空格或中文。

如果虚拟环境损坏、复制自另一台电脑，或者 Python 安装发生过变化，可以运行：

```powershell
run_app.bat -Repair
```

启动器会保留旧环境为 `.venv.broken-时间戳`，然后自动重建。首次安装失败时，请先检查网络或代理设置，再重新运行；失败窗口会保留并显示具体原因。
...
2. 在“视觉”选项中选择 `增强处理` 或 `不处理（对照）`；画面打开后仍可切换，切换后需重新框选目标。正式 A/B 对照建议从同一视频起点分别运行两组。
